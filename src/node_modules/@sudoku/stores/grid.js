...
function createGrid() {
	...
	return {
		...
		// 新增：直接从 sequence 更新 grid
		updateFromSequence: (sequence) => {
			grid.update(() => {
				...
				for (let y = 0; y < SUDOKU_SIZE; y++) {
					...
					for (let x = 0; x < SUDOKU_SIZE; x++) {
						const cellIndex = y * SUDOKU_SIZE + x;
						const sequenceChar = sequence.charAt(cellIndex);
						
						if (sequenceChar && sequenceChar !== '0' && sequenceChar !== '.') {
							newGrid[y][x] = parseInt(sequenceChar, 10);
						} else {
							newGrid[y][x] = 0;
						}
					}
